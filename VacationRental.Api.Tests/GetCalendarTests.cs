...
namespace VacationRental.Api.Tests
{
    ...
    public class GetCalendarTests
    {
        ...
        [Fact]
        public async Task GivenCompleteRequest_WithPreparationDay_WhenGetCalendar_ThenAGetReturnsTheCalculatedCalendar()
        {
            var postRentalRequest = new RentalBindingModel
            {
                Units = 1,
            ...
            using (var getCalendarResponse = await _client.GetAsync($"/api/v1/calendar?rentalId={postRentalResult.Id}&start=2000-01-01&nights=5"))
            {
                ...
                Assert.Contains(getCalendarResult.Dates[1].PreparationTimes, x => x.Unit == 1);
